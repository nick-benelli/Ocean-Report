...
def send_email(
    *,
    ...
    logger.debug("    ✓ Email message constructed in %.3f seconds", time.time() - operation_start)
    ...
    try:
        with smtplib.SMTP(smtp_server, smtp_port) as server:
            ...
            logger.debug("    ✓ SMTP connection established in %.2f seconds", connect_time)
            
            logger.debug("    → Starting TLS upgrade...")
            tls_start = time.time()
            server.starttls()  # Upgrade the connection to secure
            logger.debug("    ✓ TLS upgrade completed in %.2f seconds", time.time() - tls_start)
            
            logger.debug("    → Authenticating with SMTP server...")
            auth_start = time.time()
            server.login(sender_email, email_password)
            logger.debug("    ✓ SMTP authentication succeeded in %.2f seconds", time.time() - auth_start)
            
            logger.debug("    → Sending email message...")
            send_start = time.time()
            server.send_message(msg)  # Send the message
            logger.debug("    ✓ Email message sent in %.2f seconds", time.time() - send_start)
        ...
        logger.info("    ✓ Complete email operation took %.2f seconds", total_operation_time)
        
    except smtplib.SMTPException as e:
        logger.error("    ✗ SMTP error after %.2f seconds: %s", time.time() - smtp_start, e)
        raise
    except Exception as e:
        logger.error("    ✗ Unexpected error during email send after %.2f seconds: %s", 
                    time.time() - smtp_start, e)
        raise
