...
def test_tide_service_error_includes_station_id(mock_context):
    ...
    with patch.object(mock_context.client, "get") as mock_get:
        ...
        assert ("8534720" in error_msg or "failed" in error_msg.lower() or 
                "validation" in error_msg.lower() or "error" in error_msg.lower())
...
def test_water_temp_service_error_includes_context(mock_context):
    ...
    with patch.object(mock_context.client, "get") as mock_get:
        ...
        assert ("8534720" in error_msg or "timeout" in error_msg.lower() or 
                "validation" in error_msg.lower() or "error" in error_msg.lower())
...
def test_pydantic_type_error_is_clear():
    ...
    assert "float" in error_msg.lower() or "number" in error_msg.lower() or "type" in error_msg.lower()
